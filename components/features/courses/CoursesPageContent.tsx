...
export function CoursesPageContent({
  ...
  }, [initialData, initialError, initialPage, initialTotalPages])
  ...
  const loadCourses = async (pageOverride = currentPage) => {
    setIsLoading(true)
    setError(null)
  ...
  return (
    <main className="flex-grow">
      <div data-role="page-content" className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 w-full">
        <div className="mb-8 sm:mb-10">
          <h1 data-role="page-title" className="text-2xl sm:text-3xl md:text-4xl font-bold text-brand-navy mb-2 sm:mb-3 font-heading">
            Courses
          </h1>
        ...
        <div data-role="course-list" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {courses.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-12 text-center md:col-span-2 lg:col-span-3">
        ...
        {totalPages > 1 && courses.length > 0 && (
          <div className="mt-8 flex justify-center gap-2">
            ...
            <span data-role="pagination-status" className="px-4 py-2 text-gray-700">
              Page {currentPage + 1} of {totalPages}
            </span>
            <button
              id="courses-pagination-next"
...
function CourseCard({ course }: { course: Course }) {
  ...
  return (
    <Link
      ...
      data-detail-uri={`/courses/${course.slug || course.courseId}`}
      className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all p-4 sm:p-6 group"
    >
      <h3 data-role="course-name" className="text-lg sm:text-xl font-bold text-brand-navy mb-2 font-heading group-hover:text-brand-blue transition-colors">
        {course.courseName}
      </h3>
      ...
      <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 mb-3">
        ...
        <span data-role="affiliation">{course.affiliation.replace(/_/g, ' ')}</span>
      </div>

      <p data-role="description" className="text-xs sm:text-sm text-gray-600 mb-4 line-clamp-3">
        {course.description}
      </p>
      ...
      <div className="flex items-center justify-between">
        {collegeCount > 0 && (
          <div className="flex items-center gap-2 text-xs sm:text-sm text-brand-blue font-medium">
            ...
            <span data-role="college-count">Available at {collegeCount} {collegeCount === 1 ? 'college' : 'colleges'}</span>
          </div>
        )}
