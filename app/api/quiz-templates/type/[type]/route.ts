...
/**
 * GET /api/quiz-templates/type/[type]
 * Proxy for: GET /api/v1/quiz-templates/type/{type}
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/v1/quiz-templates/type/${encodeURIComponent(type)}?${queryParams.toString()}`,
      {
        method: 'GET',
        headers,
        cache: 'no-store',
    ...
    if (!response.ok) {
      logger.error('[API] Backend error fetching quiz templates by type:', {
    ...
    logger.error('[API] Error fetching quiz templates by type:', error)
    return NextResponse.json(
