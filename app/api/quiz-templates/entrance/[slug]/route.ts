...
/**
 * GET /api/quiz-templates/entrance/[slug]
 * Proxy for: GET /api/v1/quiz-templates/entrance/{slug}
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/v1/quiz-templates/entrance/${encodeURIComponent(slug)}?${queryParams.toString()}`,
      {
        method: 'GET',
        headers,
        cache: 'no-store',
    ...
    if (!response.ok) {
      logger.error('[API] Backend error fetching quiz templates by entrance:', {
    ...
    logger.error('[API] Error fetching quiz templates by entrance:', error)
    return NextResponse.json(
